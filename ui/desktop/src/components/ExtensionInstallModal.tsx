...
export function ExtensionInstallModal({ addExtension, setView }: ExtensionInstallModalProps) {
  ...
  const handleExtensionRequest = useCallback(async (link: string): Promise<void> => {
    if (processingLinkRef.current === link) {
      console.debug(`Skipping duplicate extension request (already processing): ${link}`);
      return;
    }
    processingLinkRef.current = link;

    try {
      console.debug(`Processing extension request: ${link}`);
      ...
      if (extensionsList?.find((ext) => ext.name === extName)) {
        console.debug(`Extension Already Installed: ${extName}`);
      ...
      console.debug('Extension not found, continuing to show modal');
  ...
  const confirmInstall = useCallback(async (): Promise<void> => {
    ...
    try {
      console.debug(`Confirming installation of extension from: ${pendingLink}`);
      ...
      if (addExtension) {
        await addExtensionFromDeepLink(
          ...
          (view: string, options?: ViewOptions) => {
            console.debug('Extension installation completed, navigating to:', view, options);
  ...
  useEffect(() => {
    console.debug('Setting up extension install modal handler');
