...
| Document | Description |
|---|---|
| [**⚡ TimeWarp Bar UI/UX Spec**](architecture/TIMEWARP_BAR_UI_UX_SPEC.md) | **THE crown jewel.** Extreme-detail UI/UX engineering specification for the TimeWarp Bar — every pixel zone, 16 event node types, 5 context menus (60+ items), 45+ keyboard shortcuts, 5 inspector tabs, 20+ animations, multi-track branch rail system, minimap, transport controls, search/filter, zoom LOD, drag-and-drop, tooltips, accessibility, theming, performance targets, and 6 complete ASCII wireframes. Fusion 360 × video editor × GitKraken × DAW transport. |
